...
Authentication tests separate identity verification (`401`) from authorization (`403`) and focus on token lifecycle and data exposure.
...
**HTTP Method:** `GET`  
**Purpose:** Verify a valid low-privilege token cannot access an admin-only resource.  
**Preconditions:** Two identities exist: normal user and admin.  
**Headers:** `Authorization: Bearer <standard-user token>`  
**Request Body:**

```json
{}
```
**Test Data:** Valid token whose role/scope does not grant admin access.  
**Expected Response:** `403 Forbidden`; no admin data returned.  
**Actual Response:** Portfolio authorization expectation: authenticated standard user receives `403`.  
**Validation Points:** Authentication remains valid, authorization denied, no object-count/PII leakage, audit event generated where required.  
**Possible Bugs:** Horizontal/vertical privilege escalation; 200 with partial admin data; role trusted from client-supplied field.  
**Priority:** Critical  
**Severity:** Critical
